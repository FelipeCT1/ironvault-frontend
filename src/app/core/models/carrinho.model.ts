...
export interface ItemCarrinho {
  ...
  bloqueadoAte?: string; // timestamp de expiração do bloqueio
}
...
export interface Cupom {
  ...
  valor: number;
  validoAte: string;
  clienteId?: number; // para cupons de troca
}

export interface OpcaoFrete {
  id: string;
  nome: string; // PAC, SEDEX, Expresso
  prazoDias: number;
  valor: number;
}
...
export interface CarrinhoTotais {
  ...
  valorFrete: number;
  total: number;
  totalAPagar: number; // total - cupons de troca
}
